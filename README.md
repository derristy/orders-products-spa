...
- **Vuex 4** — глобальное состояние (модули `orders`, `products`)
- **Vue Router 4** — роутинг с lazy-загрузкой страниц и анимацией переходов
...
| Метод  | Путь                | Описание                          |
| ------ | ------------------- | --------------------------------- |
| GET    | `/api/orders`       | список приходов с продуктами      |
| GET    | `/api/orders/:id`   | один приход                       |
| DELETE | `/api/orders/:id`   | удалить приход                    |
| GET    | `/api/products`     | все продукты (`?type=` — фильтр)  |
